            ...
            tokio::select! {
                ...
                Some(input) = input_rx.recv() => {
                    match input {
                        ...
                        _ => { terminal.status_set(UiStatusMode::Normal) },
                    }
                }
            }
            ...
            .stream_input(input.filter_map(move |i| {
                ...
                async move {
                    ...
                    match i {
                        ...
                        Input::Bytes(data) => {
                            // Notify terminal about input event, as it might
                            // change the current status mode.
                            input_tx.send(Input::Dummy).await.unwrap();
                            Some(data)
                        }
                        _ => None,
                    }
