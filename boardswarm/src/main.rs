...
use boardswarm_protocol::{
    ...
    ItemPropertiesRequest, ItemTypeRequest, LoginInfoList, Property, VolumeCommitRequest,
    VolumeEraseRequest, VolumeInfoMsg, VolumeIoTargetReply, VolumeRequest,
};
use bytes::Bytes;
use clap::Parser;
use futures::prelude::*;
...
pub trait Volume: std::fmt::Debug + Send + Sync {
    ...
    async fn commit(&self, _target: &str) -> Result<(), VolumeError>;
...
impl boardswarm_protocol::boardswarm_server::Boardswarm for Server {
    ...
    async fn volume_commit(
        &self,
        request: tonic::Request<VolumeCommitRequest>,
    ) -> Result<tonic::Response<()>, tonic::Status> {
        let request = request.into_inner();
        let volume = self
            .get_volume(request.volume)
            .ok_or_else(|| tonic::Status::not_found("Volume not found"))?;
        volume.commit(&request.target).await?;
        Ok(tonic::Response::new(()))
    }
