...
impl DeviceConfigItem for config::Console {
    #[instrument(fields(name = self.name), skip(self, properties))]
    fn matches(&self, properties: &Properties) -> bool {
        if self.match_.is_empty() {
            warn!("Console matches is empty - could match random device");
        }
...
impl DeviceConfigItem for config::Volume {
    ...
    fn matches(&self, properties: &Properties) -> bool {
        if self.match_.is_empty() {
            warn!("Volume matches is empty - could match random device");
        }
        properties.matches(&self.match_)
    }
}
...
impl DeviceConfigItem for config::ModeStep {
    fn matches(&self, properties: &Properties) -> bool {
        if self.match_.is_empty() {
            warn!("ModeStep matches is empty - could match random device");
        }
        properties.matches(&self.match_)
    }
}
