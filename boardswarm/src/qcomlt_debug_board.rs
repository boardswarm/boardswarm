...
async fn setup_subdevices(
    ...
    // TODO: power measurement (voltage, current). The syntax is:
    /*
    0mV 0mA
...
// TODO add other debug board commands
...
/*
TODO: calling change-mode in cli directly does not work (but setting mode does). Does it also not work for the other implementations ?

obbardc@nox:~/projects/git/boardswarm$ RUST_LOG=info cargo run --bin boardswarm-cli -- -i localhost actuator 1 change-mode on
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 0.13s
     Running `target/debug/boardswarm-cli -i localhost actuator 1 change-mode on`
