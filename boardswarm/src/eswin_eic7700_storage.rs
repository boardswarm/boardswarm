        ...
        match d {
            DeviceEvent::Add { device, seqnum } => {
                ...
                tokio::spawn(async move {
                    if let Err(e) = setup_volume(prereg, properties).await {
...
async fn setup_volume(r: PreRegistration, properties: Properties) -> anyhow::Result<()> {
    ...
    let state_dir = tokio::fs::canonicalize(
        systemd_directories::state_dir()
            .or(dirs::state_dir())
            .ok_or_else(|| anyhow::anyhow!("Failed to get systemd state directory"))?,
    )
    .await?;
    let path = state_dir.join(id);
    tokio::time::timeout(
        MOUNT_TIMEOUT,
        wait_mount(
            Path::new(devname),
...
impl EswinEic7700StorageVolume {
    ...
    fn target_path(&self, target: &str) -> Result<PathBuf, VolumeError> {
        let (name, filename) = TARGET_BOOTLOADER;
        if name == target {
...
impl Volume for EswinEic7700StorageVolume {
    ...
    async fn commit(&self) -> Result<(), VolumeError> {
        // TODO: sync filesystem? wait for umounted?
        Ok(())
    }
}
